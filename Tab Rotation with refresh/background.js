...
async function startTabRotation(intervalSeconds, refreshEnabled = false) {
  ...
  isRunning = true;
  console.log(`Starting tab rotation with ${intervalSeconds} second interval${refreshEnabled ? ' (with refresh)' : ''}`);
    ...
    rotationTimer = setInterval(async () => {
      try {
        ...
        if (nextTab && nextTab.id) {
          // Refresh tab if refresh is enabled
          if (refreshEnabled) {
...
chrome.runtime.onStartup.addListener(() => {
  chrome.storage.sync.get(['isRunning', 'interval', 'refreshEnabled'], (result) => {
